...
export default function SequenceFormModal({ sequence, onClose, onSubmit }) {
  const [niveaux, setNiveaux] = useState([])
  const [annees, setAnnees] = useState([])
  const [loadingOptions, setLoadingOptions] = useState(true)

  const [niveauId, setNiveauId] = useState(sequence?.niveau_id ?? '')
  const [anneeAcademiqueId, setAnneeAcademiqueId] = useState(sequence?.annee_academique_id ?? '')
  const [numero, setNumero] = useState(sequence?.numero ?? 1)
  const [libelle, setLibelle] = useState(sequence?.libelle ?? '')
  ...
  return (
    <Modal title={sequence ? 'Modifier la séquence' : 'Nouvelle séquence'} onClose={onClose}>
      {loadingOptions ? (
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <Input id="libelle" label="Libellé" value={libelle} onChange={(e) => setLibelle(e.target.value)} required placeholder="Séquence 1" />
